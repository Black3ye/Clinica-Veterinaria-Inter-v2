...
Public Class Site2
    ...
    Private Sub btnadd_Click(sender As Object, e As EventArgs) Handles btnadd.Click
        Response.Redirect("MVadd.aspx?UID=" & Global_class.UserID & "&UN=" & Global_class.UserName & "&NI=" & Global_class.ID & " ")
    End Sub

    Private Sub btnSEdit_Click(sender As Object, e As EventArgs) Handles btnSEdit.Click
        Response.Redirect("MVSEdit.aspx?UID=" & Global_class.UserID & "&UN=" & Global_class.UserName & "&NI=" & Global_class.ID & " ")
    End Sub
    ...
    Private Sub Report_Click(sender As Object, e As EventArgs) Handles Report.Click
        Evaluate()
        If Global_class.role = "Administrator" Then
            Dim s As String = "window.open('Reportes/ReportInvoice.aspx?UID=" & Global_class.UserID & "&UN=" & Global_class.UserName & "&NI=" & Global_class.ID & " ', '_blank');"
    ...
    Private Sub btnDV_Click(sender As Object, e As EventArgs) Handles btnDV.Click
        Response.Redirect("MVDV.aspx?UID=" & Global_class.UserID & "&UN=" & Global_class.UserName & "&NI=" & Global_class.ID & " ")
    End Sub

    Private Sub Report1_Click(sender As Object, e As EventArgs) Handles Report1.Click
        Evaluate()
        If Global_class.role = "Administrator" Then
            Dim s As String = "window.open('Reportes/ReportDV.aspx?UID=" & Global_class.UserID & "&UN=" & Global_class.UserName & "&NI=" & Global_class.ID & " ', '_blank');"
            Page.ClientScript.RegisterStartupScript(Me.GetType(), "alertscript", s, True)
        Else
            Page.ClientScript.RegisterStartupScript(Me.GetType(), "alert", "alert('Usted no es Administrador');", True)
        End If
